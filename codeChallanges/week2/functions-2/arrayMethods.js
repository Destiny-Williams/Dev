...
// CODE HERE
const evenNumbers // = mixedNumbers.filter(/* 6, 2, 6, 8, 4, 2, 8, 4 */)
...
//  const evenNumbers = mixedNumbers.filter (wholeArray => wholeArray % 2 === 0)
...
//   return (num % 2 == 0)
// }
// mixedNumbers = mixedNumbers.filter(evenNumbers);
// console.log(mixedNumbers);

constecenNu  = mixedNumbers.filter((num) => {
  if (num % 2 ==0){
    return true;
  } else {
    return false;
...
//const mixedNumbers = mixedNumbers.filter(mixedNumbers => mixedNumbers % 2 == 0);
 console.log(evenNumbers());
...
*/
//lets break it down:
//write a metod that writes and array // we need a higher order method. or to have a call back order. 
//we need our call back function to take in two numbers. for price. and for tax rate. and then returns a single number. 
// the number we put out will need to represent the orders final price. 
// loop though the array, running the call back on each element. 
// push the new number/array of final prices 
...
//higherFunction(orders,calculateFinalCost)
console.log(results)
